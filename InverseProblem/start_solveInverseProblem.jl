...
@everywhere begin
    batch_job = false
    constraints = [:NONNEG]
    coordinate_system = "(r,z)" # Specify as e.g. "(E,p)", "(vpara,vperp)", "(E,pm,Rm)" or "(E,Lambda,Pphi_n)"
    excluded_measurement_intervals = ["",""]
    excluded_measurement_units = ["m"," m"]
    ...
    filepaths_S = ["../my_code/output/measurements_S_50.jld2"]
    filepaths_W = ["../my_code/output/a_50.jld2"]
    folderpath_out = "../my_code/output/inverse/"
    gif_solutions = false
    min_array = []
    min_array_units = []
    max_array = []
    ...
    plot_solutions = true
    nr_array = [] # Should be equal in length to 'regularization'
...
include("/home/francesco/Documenti/Master/Blok4/PUK/OWCF/solveInverseProblem.jl")
